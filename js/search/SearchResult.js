...
class SearchResult {
  ...
  // Display union between searchTagsRecipesResult and searchMainRecipesResult
  // if tags and main input are not empty
  static displayResult(searchMainRecipesResult, searchTagsRecipesResult, searchParams) {
    ...
    if (searchParams.appareils.length > 0
      ...
      if (searchParams.main.length > 0) {
        displayedResult = searchTagsRecipesResult.filter(
          (x) => searchMainRecipesResult.includes(x),
        );
      } else {
